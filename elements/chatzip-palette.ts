...
export class PaletteElement extends HTMLElement {
    ...
    constructor() {
        ...
        for (let i = 0, j = 8; i < j; i++) {
            ...
            this.paletteList[i].addEventListener('click', () => {
                if (this.paletteList[i].id === 'eraser') {
                    this.selectColor(-1);
                    return;
                }
                // this.selectColor(i);
        ...
        const paletteColors = new Array<HTMLSpanElement>();
        for (let i = 1; i < 65 ; i++) {
            paletteColors[i] = document.createElement('span') as HTMLSpanElement;
            paletteColors[i].style.backgroundColor = palette.colors[i];
            paletteColors[i].addEventListener('click', () => {
                if (palette.selected === -1) return;
                palette.list[palette.selected] = i;
                this.paletteList[palette.selected].style.backgroundColor = palette.colors[i];
        ...
        this.wrapper.append(...this.paletteList, this.eraser, this.brush);
